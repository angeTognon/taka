<?php
// filepath: /Users/mac/Documents/taka2/moneroo_publish_init_fixed.php
...
// Log pour debug
error_log("Moneroo Publish Init - Devise: $currency, Pays: $country");
...
$description = isset($input['description']) ? $input['description'] : "Publication livre";
...
// Construction des metadata pour la publication
$metadata = [
    "user_id" => $user_id,
    "currency" => $currency,
    "country" => $country,
...
// Ajouter les données du livre si disponibles
if (isset($input['title']) && !empty($input['title'])) {
    $metadata['book_title'] = $input['title'];
}
if (isset($input['genre']) && !empty($input['genre'])) {
    $metadata['book_genre'] = $input['genre'];
}
if (isset($input['language']) && !empty($input['language'])) {
    $metadata['book_language'] = $input['language'];
}
...
error_log("Moneroo Publish Init - Données envoyées: " . json_encode($data));
