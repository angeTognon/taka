...
$stmt = $pdo->prepare("SELECT COUNT(*) as totalSales, COALESCE(SUM(amount),0) as totalRevenue 
                       FROM taka_sales WHERE user_id = ?");
$stmt->execute([$user_id]);
...
// Lecteurs uniques
$stmt = $pdo->prepare("SELECT COUNT(DISTINCT buyer_id) 
                       FROM taka_sales WHERE user_id = ?");
$stmt->execute([$user_id]);
...
while ($book = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $book_id = $book['id'];
    $s = $pdo->prepare("SELECT COUNT(*) as sales, 
                               COALESCE(SUM(amount),0) as revenue, 
                               COUNT(DISTINCT buyer_id) as readers, 
...
$stmt = $pdo->query("SELECT plan, COUNT(*) as count 
                     FROM taka_subscriptions 
                     WHERE status = 'actif' 
                     GROUP BY plan");
$subsStats = [];
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $subsStats[$row['plan']] = (int)$row['count'];
}
...
$stmt = $pdo->prepare("SELECT COALESCE(AVG(rating),0) 
                       FROM taka_sales 
                       WHERE user_id = ?");
$stmt->execute([$user_id]);
$avgRating = round((float)$stmt->fetchColumn(), 1);

// Taux d'achèvement (dummy)
$completionRate = 0.78;
